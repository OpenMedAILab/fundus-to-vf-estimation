...
from config import SRC, CKPT, RESULTS, EXT_CFP as EXT
...
json.dump(out,open(f"{RESULTS}/external_pred_ms.json","w"),indent=1,ensure_ascii=False)
...
# 对比GRAPE测试集预测MS分布
g=[json.load(open(f)) for f in glob.glob(f"{CKPT}/reg_hybrid_cfp_s0/result.json")][0]
