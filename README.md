...
```
src/                     # 全部 Python 代码
  config.py              # 统一路径配置（ROOT/数据/输出，环境变量可覆盖）
  repro.py               # 核心训练（数据集/模型/训练/评估，自包含）
  orchestrate.py         # 多 GPU 并行编排 (task×arch×input×seed)
  warm.py  build_aug.py  # 预热下载权重 / 构建增广(合成)样本索引
  run_extra.py           # 补充实验批处理
  aggregate.py  report.py  append_*.py                 # 汇总 result.json、生成报表
  clinical_analysis.py  grading_dca.py  longitudinal_analysis.py
  gradcam.py  fid_kid.py  plot_convergence.py  make_supp_figures.py   # 分析与配图
  external_*.py  ext_domain.py  wannan_ext_val.py       # 外部验证 (inference-only)
...
results/                 # 评估输出
  ...
  logs/                  # 训练日志
  wannan_extval/         # 皖南外部验证结果 (json + 图)
  *.json                 # aggregate_summary / clinical_analysis / fid_kid …

figures/                 # 论文正文配图 (*.png) + supp_fig/ 附录图
reports/                 # 复现/实验报告 (markdown)
docs/                    # 数据结构说明.md
```
...
# 单个 run
python src/repro.py --task reg --arch full_hybrid --input cfp --seed 0 [--aug]

# 全量并行（按 GPU 分发全部配置）
python src/orchestrate.py all --seeds 0,1,2 --gpus 0,1,2,3

# 汇总结果
python src/aggregate.py && python src/report.py
```

> 所有路径集中在 **`src/config.py`**：仓库根目录自动作为 `ROOT`（`data/`、`results/`、`figures/`、`reports/` 由此派生），数据集等外部路径通过环境变量覆盖（各有默认值）。无需再手改任何脚本。
...
| `results/ckpt/**/best.pth` | 训练得到的模型权重（约 35 GB），仅保留了 `result.json` 指标 |

## ⚠️ 重要提示

1. **路径配置**：所有脚本的数据/输出路径已统一到 `config.py`，由环境变量覆盖（见「快速开始」），
   不再有指向特定服务器的硬编码绝对路径。
2. **患者隐私**：外部验证结果文件（`results/external_ms_pairing.json`、`results/external_pred_ms.json`、
   `results/wannan_extval/wannan_extval_pairs.json`）中的真实受试者姓名已**脱敏为匿名 ID**（`P001`…），
