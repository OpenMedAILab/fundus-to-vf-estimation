...
for arch in ["resnet","full_hybrid"]:
    ...
    for v in ["ce_nosamp","ce_samp","focal_samp"]:
        rs=[json.load(open(f)) for f in glob.glob(R+"/ckpt/cls_"+arch+"_cfp_"+v+"_s*/result.json")]
...
ls=json.load(open(R+"/longitudinal_stats.json"))
...
gd=json.load(open(R+"/grading_dca.json"))
